...
function check_if_took (data, intervals){
    var time_now = Math.floor(Date.now() / 1000) + 3600;//UK time
    var counter = 0;
    ...
    for (var i = 0; (i < data.length) && (counter < intervals.length); i++){
        ...
        var start_mins = (intervals[counter].start -moment().startOf('day').unix() - 3600) / 60;
        var start_hours = Math.floor(start_mins/60);
        var start_minutes = start_mins % 60;
        if (start_minutes == 0){
            start_minutes = '00';
        }
        var end_mins = (intervals[counter].end - moment().startOf('day').unix() - 3600) / 60;
        ...
        if ((intervals[counter].start <= data[i].creationTimestamp) && (intervals[counter].end >= data[i].creationTimestamp)){
            counter++;
            console.log('You took pill for time interval: ' + start_hours+':'+start_minutes+'-'+end_hours+':'+end_minutes);
        } else if (data[i].creationTimestamp < intervals[counter].start){
            console.log('Took pill at wrong time');
        }
    }
...
function getStartTime() {
    ...
    return moment().startOf('day').unix() + seconds + 3600;
}
